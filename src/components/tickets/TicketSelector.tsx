...
import { getUserBalance, buyTickets } from "@/app/actions/user"
...
export function TicketSelector({ pricePerTicket }: { pricePerTicket: number }) {
    ...
    const handleBulkAdd = () => {
        ...
        let attempts = 0
        while (newNumbers.length < qty && attempts < qty * 5) {
            const num = Math.floor(1000 + Math.random() * 9000).toString()
            if (!selectedNumbers.includes(num) && !newNumbers.includes(num)) {
                newNumbers.push(num)
            }
            attempts++
        }

        if (newNumbers.length < qty) alert(`Solo se pudieron generar ${newNumbers.length} únicos.`)
            ...
            <CardContent className="space-y-6 pt-6">
                ...
                <div className="flex gap-2">
                    <Input
                        ...
                        className="text-2xl font-mono tracking-widest text-center bg-zinc-900 border-zinc-700 h-14"
                        onKeyDown={(e) => e.key === "Enter" && handleAddNumber()}
                    />
                    <Button onClick={handleAddNumber} className="h-14 w-14 bg-zinc-800 hover:bg-zinc-700 border border-white/10">
